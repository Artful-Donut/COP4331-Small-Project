...
function fetchContacts() {
    ...
    fetch("http://23.20.217.81/contactManagerFolder/LAMPAPI/CoreFunctionsAPI/ReadContact.php", {
        method: 'POST',
...
function createContact()
{
    ...
    if (fullName && email && phone)
    {
        ...
        fetch("http://23.20.217.81/contactManagerFolder/LAMPAPI/CoreFunctionsAPI/CreateContact.php", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: jsonPayload
        })
...
function updateContact() {
    if (selectedContact !== null) {
        ...
        if (newFullName && newEmail && newPhone) {
            ...
            fetch("http://23.20.217.81/contactManagerFolder/LAMPAPI/CoreFunctionsAPI/UpdateContact.php", {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: jsonPayload
            })
...
function deleteContact()
{
    if (selectedContact !== null)
    {
        ...
        if (confirmDelete)
        {
            ...
            // Fetch Request -> Need to pass through the ID, UserID, new name, new email, and new phone number
            fetch("http://23.20.217.81/contactManagerFolder/LAMPAPI/CoreFunctionsAPI/DeleteContact.php", {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                body: jsonPayload
            })
